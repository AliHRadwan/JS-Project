...
function normalizeFromUserDoc(raw={}){
  ...
  const size = raw.size  || "";
  return { id, title, price, image, color, size, quantity: qty, addedAt: raw.addedAt || Date.now() };
}
...
// =================================================================
// New function to check product stock in Firestore
async function checkStock(productId, requestedQty) {
  ...
  try {
    ...
    // Assumes stock is stored in a field named 'stock'
    const availableStock = Number(productData.stock);
    
    if (isNaN(availableStock)) {
        console.warn(`Product ${productId} has an invalid 'stock' field.`);
        return false;
    }
...
document.addEventListener("click", async (e) => {
    ...
    if (btn.classList.contains("remove-item")) {
        ...
        if (btn.classList.contains("plus")) {
            ...
            const hasStock = await checkStock(productId, newVal);
...
document.addEventListener("change", async (e) => {
    ...
    const requestedQty = clampQty(parseInt(input.value, 10));

    // Find the item's current quantity in the local cart cache
    const itemInCart = cachedCart.find(x => x.id === productId && (x.color || "") === color && (x.size || "") === size);
    const currentQty = itemInCart ? itemInCart.quantity : 0;

    // Only check stock if the user is increasing the quantity
    if (requestedQty > currentQty) {
        const hasStock = await checkStock(productId, requestedQty);
        if (hasStock) {
        ...
        // If quantity is decreased or the same, just update it
        await setQty(li, requestedQty);
    }
});
